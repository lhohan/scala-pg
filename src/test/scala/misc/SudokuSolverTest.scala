...
import misc.SudokuSolver._
  ...
  def time[A](x: => A):(Long, A) = {
    val start = System.currentTimeMillis()
    val result = Await.result(Future(x), 10 seconds)
    val end = System.currentTimeMillis()
    (end - start, result)
  }

  test("solve - version one of solver, original puzzle") {
    assertResult(solved) {
      val (elapsed, result) = time(solve(Stream(
      ...
      printTime(elapsed, "version one of solver, original puzzle")
      result.head
    }
  }
